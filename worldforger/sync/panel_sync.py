...
- characters: summary, design_notes, entities[] (每项 id, name; 可选 cast_role, faction_ids[], home_region_id, one_line_hook, notes, notable_skills[], relations[]；新增/修改角色时必须保留 age, gender, power_tier, profession_id, attributes, inventory[], skills[]。skills[] 为对象数组：name, description, exclusive(bool 是否专属), source(可选，技能树节点 id), level(可选熟练度))。
...
关键约束：提取 characters.entities[] 时，必须保留 age, gender, power_tier, profession_id, attributes, inventory[], skills[] 等字段，不要精简或省略。"""
