...
2. 顶层键**只能**来自：geography, ecology, power_system, item_quality_system, attribute_system, factions, cultures, characters, history, economy。未涉及或无法可靠抽取的模块**不要出现**该键。**不得**输出 `files`、`search` 等工作台键（「导出与快照」「全文搜索」见对话上下文中 **studio** 说明，非 world.json 持久化字段）。
